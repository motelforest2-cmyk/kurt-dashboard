// ============================================================
// lib/flow.js — Logica post-connessione (il porting di .kurt sul web)
// ============================================================
...
// dell'utente (self-chat), come fa demoPairManager del Kurt vero:
...
// Canale  KURT (contextInfo) — stesso branding del bot vero.
const kurtChannel = {
    forwardingScore: 1,
    isForwarded: true,
...
async function inviaBivio(sock, myJid) {
    const testo =
        `🤔 *Vuoi l'installazione automatica su WispByte?*\n\n` +
    ...
    try {
        await sock.sendMessage(myJid, {
            ...
            footer: 'Kurt Cloud ☁️',
            buttons: [
                { buttonId: 'kurt_si', buttonText: { displayText: '✅ Sì, automatica' }, type: 1 },
                { buttonId: 'kurt_no', buttonText: { displayText: '📄 No, manuale' }, type: 1 }
            ],
            headerType: 1,
...
async function ramoAutomatico(sock, myJid, creds, send) {
    await sock.sendMessage(myJid, {
        text:
            ...
            `1️⃣ Avvia PRIMA il tuo server (WispByte) col file di installazione — così il codice sarà già pronto in console (o via mail, se l'hai registrata).\n` +
            `2️⃣ Poi mandami qui il *codice di abbinamento* (es. \`A1B2-C3D4\`).\n\n` +
            `⏳ Hai 15 minuti prima che le credenziali in attesa scadano.`,
        contextInfo: kurtChannel
    });
...
async function ramoManuale(sock, myJid, credsBuffer, send) {
    ...
    await sock.sendMessage(myJid, {
        text:
            `📖 *GUIDA RAPIDA*\n\n` +
            `1️⃣ Scarica il pacchetto KURT e caricalo sul tuo hosting.\n` +
            `2️⃣ Metti il file \`creds.json\` qui sopra nella cartella \`session\`.\n` +
            `3️⃣ Avvia il bot.\n\n` +
            `Per assistenza contatta *Kurt*.`,
        contextInfo: kurtChannel
    });
...
async function runPostConnect(sock, ctx) {
    ...
    try {
        ...
        await sock.sendMessage(myJid, {
            text: `✅ *Collegato!*\n\nSono Kurt Cloud ☁️. Ora scegli come vuoi installare il bot.`,
            contextInfo: kurtChannel
        });
